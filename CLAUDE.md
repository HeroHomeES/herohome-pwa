...
- **Resto de campos de propiedad (hecho en esta misma sesión):** `create-user` ahora persiste también `description`, `age`, `communityFee→community_fee`, `condition`, `electronicCertificate→electronic_certificate`, `external→is_exterior`, `garageSpace→garage_space`, `heatingType→heating_type`, `elevator→has_elevator`, `orientation`, `refCatastral→ref_catastral`, `registroPropiedad→registro_propiedad`, `rejectOffersBelow→reject_offers_below` y `floor` (antes el Apex los enviaba pero se ignoraban). `high` (`High__c`) NO tiene columna en `properties` → se omite. ⚠️ Pendiente de verificar que los valores de picklist de SF (condition/orientation/heating_type/garage_space/electronic_certificate/housing_type/status) coinciden exactamente con las etiquetas que espera la PWA en sus `SelectField`; si no, se guardan pero el desplegable no los reconoce.
