...
- **Pendiente (no código)**: aprobar plantilla `bienvenida_pc` en Meta y reconfigurar Make Esc. 2 — ambos del flujo de captación de Idealista (`process-idealista-lead`), aún sin probar end-to-end. Mejora menor anotada: afinar el prompt de Hero en la rama "sin vivienda asociada" (no debe pedir el enlace del anuncio, que no sabe procesar).
