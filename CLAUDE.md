...
**Dos entregas (pendiente: aplicar SQL + push + validar e2e).**

- **Condiciones especiales (`properties.special_conditions`):** nueva columna `text` nullable, editable a mano en el Table Editor. Si está rellena (p.ej. "vivienda VPO…", "no se vende antes de diciembre 2026"), el `whatsapp-agent` la inyecta en el system prompt con la instrucción de informar al comprador **citándola tal cual** la PRIMERA vez que muestre interés en visitar, ANTES de mostrar horarios, y preguntarle si sigue interesado (si no le encaja, despedida sin insistir; dudas sobre las condiciones → escalado a humano). Vacía/NULL = comportamiento idéntico al actual. **Decisión de diseño:** vía prompt (informativo), NO gate determinista — el gate verbatim se reserva para consentimientos con trazabilidad legal (honorarios); si en el futuro se quiere registrar "fue informado y aceptó", convertir en gate. Cambios: select + `buildSystemPrompt` en `whatsapp-agent/index.ts`.
- **Sección "Conversaciones WhatsApp" en el dashboard admin** (`admin/index.html`): el usuario estaba ciego ante las conversaciones Hero↔PC (veía la fila en `whatsapp_conversations` pero no el hilo). ⚠️ **Aclarado: WhatsApp Web NO es opción** — un número dado de alta en Cloud API queda desvinculado de la app de consumo (ni WhatsApp Web ni bandeja de Meta Business Suite). La solución: nueva sección en admin.herohome.es entre "Para hoy" y "Viviendas": lista de conversaciones ordenadas por `last_message_at` (teléfono, nombre del comprador si se conoce por visitas/ofertas, vivienda, nº mensajes) y desplegable (`<details>`) con vista tipo chat (burbujas comprador/Hero + timestamp `ts` de cada mensaje). Requiere política RLS SELECT admin sobre `whatsapp_conversations` (antes DENY total) reutilizando `is_admin()`. Verificado visualmente en local con datos de demostración (login sin errores, chat renderiza bien).
- **SQL nuevo (aplicar manual):** `supabase/sql/2026-07-14-special-conditions-y-conversaciones.sql` (ALTER TABLE + política RLS). `docs/FUNCIONAMIENTO.md` actualizado (§4.5 paso 2 nuevo y §10).
- **⏳ Pendiente al cierre:** (1) aplicar el SQL en el SQL Editor; (2) push a `main` (deploy de `whatsapp-agent` vía Action + dashboard vía Vercel); (3) validar e2e: rellenar `special_conditions` en una vivienda y conversar por WhatsApp; abrir admin.herohome.es y ver las conversaciones reales.
...
**Solo prompts + enlace por defecto del asesor; CERO cambios de lógica/tools.** Los dos agentes de Hero (`whatsapp-agent` para el PC y `chat-with-hero` para el CV) reciben dos ajustes de comportamiento vía system prompt.

- **Escalado a humano / "no lo sé":** regla nueva en ambos prompts. Si el usuario pide hablar con una persona, o pregunta algo que Hero NO puede responder con sus tools ni con la info de la conversación (detalles del contrato de arras, cuestiones legales/fiscales de la compraventa, o cualquier dato que no tenga), **no se lo inventa** y le da una salida: en WhatsApp (PC) **pega el enlace** de agenda; en la PWA (CV) le **orienta a la sección "Mi Equipo"** (icono junto a la campana) para agendar la llamada con su agente, con el enlace como alternativa.
- **Contexto funcional del proceso Herohome:** bloque "Cómo funciona comprar/vender con Herohome" en cada prompt, adaptado al interlocutor. Cubre: disponibilidad (el CV la edita cuando quiera en su sección; genera los huecos reservables), visitas (reserva PC → confirma CV → aviso automático), ofertas (las decide el propietario) y que **tras una oferta aceptada un agente humano de Herohome contacta a comprador y vendedor para los siguientes pasos, incluida la firma del contrato de arras**. No repite % de honorarios (eso lo cubre el gate determinista).
- **Enlace del asesor unificado:** el enlace de "solicitar llamada" pasó del antiguo `calendar.app.google/PuJQpTUbAmTX5hjk8` al nuevo `calendar.google.com/calendar/appointments/schedules/AcZssZ3…`. Estaba **hardcodeado en 3 sitios** (ahora los tres al mismo valor): `whatsapp-agent` (nueva const `HUMAN_CALL_URL`), `chat-with-hero` (`AGENT_CALENDAR_URL`) y `src/pages/TeamPage.tsx` (`DEFAULT_AGENT_CALENDAR_URL`, botón de Mi Equipo). ⚠️ Si se cambia, tocar los 3. Una vivienda puede seguir sobreescribirlo con su `properties.agent_calendar_url` (en el agente PWA y en el botón; el `whatsapp-agent` usa siempre la const). **Hallazgo:** en la BD las 2 viviendas tienen `agent_name`/`agent_calendar_url` en `null`, así que TODO caía al default del código — el botón de Mi Equipo estaba usando el enlace antiguo, no el nuevo, hasta este cambio. Fallback de "Mi Equipo": nombre → `Alejandro Yuste`, foto → inicial (no hay default de foto), calendario → default.
- **Lint:** los errores Deno `any`/`_e` de las Edge Functions son **preexistentes** (deuda conocida), ajenos a este cambio; `src/` limpio.
- **Ficheros:** `supabase/functions/whatsapp-agent/index.ts`, `supabase/functions/chat-with-hero/index.ts`, `src/pages/TeamPage.tsx`. Deploy: push a `main` (GitHub Action despliega las 2 funciones + Vercel la PWA). **Pendiente de validar por el usuario** en WhatsApp y en el chat de la PWA tras el deploy.
...
**Rediseño completo de la capa visual de la PWA — solo diseño, CERO cambios de lógica/funcionalidad.** Motivo: la UI tenía un aspecto genérico y usaba un azul `#2E5EA1` + emojis que NO son de la marca. Ahora alineada con el brandbook. **Build y lint (`src/`) en verde; login verificado en navegador. Falta: validación del usuario en páginas autenticadas y push a `main` (= deploy) — al cierre aún sin pushear.**

- **Brandbook recuperado y consolidado:** el `DESIGN.md` v2 vivía solo como artefacto en un chat de claude.ai; el logo definitivo estaba en un proyecto de **Claude Design** ("Logo Herohome"). Ambos importados a través del Chrome del usuario (API interna `claude.ai/design/…/GetFile`). Guardados en el repo: **`DESIGN.md` v3.0** (raíz) y **`docs/brandbook/herohome-brandbook-standalone.html`** (fuente canónica, 5 páginas navegables). Mockups iterados en `docs/mockups/redesign-v{1,2}.html`.
- **Sistema de diseño:** violeta `#5B5CFF` (acento único), ink `#0A0E17`, teal `#0EA5A0` (éxito), surface `#F8FAFC`. Inter (+ Space Mono solo para metadatos). Bordes 1px en vez de box-shadows (convención Stripe/Linear). Iconos de trazo estilo Lucide en vez de emojis.
- **Logo "Pulse":** dos barras verticales redondeadas con gradiente violeta; la derecha más corta y desplazada abajo (asimetría intencional — NO "corregirla"). Reemplaza al "nodo-hogar" de un v2 obsoleto. Variantes: gradiente (fondo claro/oscuro) y barras blancas semitransparentes (sobre violeta).
- **Nuevos ficheros front:** `src/components/HerohomeLogo.tsx` (símbolo Pulse + lockup), `src/components/icons.tsx` (18 iconos Lucide con `currentColor`). Tokens de marca en `src/index.css` vía `@theme` (Tailwind 4).
- **Navegación:** el menú hamburguesa + sidebar se ELIMINAN. Ahora: **cabecera oscura `#111827`** (52px, logo + icono Equipo + campana) y **bottom tab bar de 4 secciones** (Hero · Vivienda · Visitas · Ofertas) con `env(safe-area-inset-bottom)` para iPhone en modo standalone. "Mi Equipo" pasó a la cabecera; **"Cerrar sesión" se movió del sidebar (borrado) a `TeamPage`** (sección "Tu cuenta"). Tab bar en PWA: es solo CSS, patrón estándar y recomendado mobile-first (no requiere app nativa).
- **Todas las páginas restyled** (misma lógica): `HomePage` (chat con burbujas nuevas + **chips de sugerencia** que envían el mensaje real a Hero, solo al inicio), `CalendarPage` (control segmentado + tarjetas con borde), `OffersPage` (ronda de negociación), `PropertyPage` (tarjeta-resumen de la vivienda + botón guardar flotando sobre la tab bar), `LoginPage` (gradiente sutil), `TeamPage`, y `Modal`/`Toast`/`Toggle`.
- **Chrome PWA:** `public/favicon.svg` + `public/icon-192.png` + `public/icon-512.png` (antes el manifest apuntaba a un icono inexistente); `index.html` con `viewport-fit=cover` y `theme-color #111827`; manifest actualizado (theme oscuro, icono maskable).
- **Email del magic link (login) rebrandeado:** ese email lo genera **Supabase Auth**, NO el código (se veía pobre, plantilla por defecto). Nueva plantilla de marca en **`supabase/templates/magic-link.html`** (clona `welcome.ts`: logo Pulse, botón `#5B5CFF`, variable `{{ .ConfirmationURL }}`). Asunto "Tu link para acceder a Herohome", cuerpo "Pulsa aquí para acceder:", botón "Ir a Herohome". **Aplicada por el usuario y verificada e2e** (Dashboard → Authentication → Emails → Magic Link; NO se tocó `config.toml` para no arriesgar otros ajustes de Auth en producción sin staging). El archivo del repo es la fuente de verdad para re-pegar.
- **⏳ Pendiente al cierre:** (1) que el usuario valide las páginas autenticadas con su sesión; (2) commit + push a `main` (deploy PWA vía Vercel) cuando dé el OK. Lint de `supabase/functions` con errores **preexistentes** (Deno `any`), ajenos a este cambio.
...
- **P6.4 — vivienda apalabrada:** `create-offer` devuelve 409 si la vivienda ya tiene una oferta `Accepted` (antes un segundo comprador podía ofertar tras el acuerdo). Hero traslada el motivo con tacto (texto en el error).
- **P6.1 — notify-visit interna:** ahora exige `x-api-key` (antes bastaba CUALQUIER JWT válido, incluida la anon key pública del bundle → se podían re-disparar notificaciones conociendo el UUID de una visita). `manage-visit` (su único llamador) envía la key.
- **P7.5 — crons a prueba de descuidos:** nueva tabla **`app_config`** (RLS DENY, editable en Table Editor) con `herohome_api_key` — **extraída automáticamente de cron.job** en la migración, sin pegarla a mano — y `setup-crons.sql` reescrito **sin placeholder**: los crons HTTP leen la key por subquery. Re-ejecutar el archivo entero ya NO puede romper nada (causa raíz del incidente del 5 jul, que rompió 3 crons con el placeholder).
- **P7.1 — dead-man's-switch (Healthchecks.io):** detecta "el cron NO corrió" (lo que alertTeam no cubre). `_shared/healthcheck.ts` (`pingHealthcheck`, best-effort/fail-open) cableado en `generate-slots` (solo modo cron), `visit-reminders` y `post-visit-followup` (ping OK al terminar; `/fail` en excepción); los 2 crons SQL (`cleanup-old-slots`, `complete-visits`) hacen ping vía `net.http_get` en su DO block. URLs en `app_config` (`healthcheck_*`, vacías = sin ping). **Pendiente operativo:** crear los 5 checks en healthchecks.io (cron 03:00/02:00/23:00/07:00 UTC y */30 con gracia ~15 min) y pegar las Ping URLs en `app_config`.
- **P7.2 — tests + fix del gate:** piezas deterministas extraídas a **`_shared/fee-gate.ts`** (`buildFeeMessage`/`classifyFeeReply`/`formatFeePercent`) y **`_shared/guardrails.ts`** (`BOOKING_CLAIM_REGEX`/`ACTION_CLAIM_REGEX`), con **19 tests** (`deno test supabase/functions/_shared/`, validados en local) y workflow CI nuevo (`.github/workflows/test.yml`). **FIX legal-UX:** las PREGUNTAS son ahora AMBIGUAS en el gate — antes "¿no incluye IVA?" contaba como RECHAZO por el token "no"; ahora repregunta (máx. 1 reintento, como siempre). Hallazgo de los tests: `Intl` es-ES **no** separa miles en cifras de 4 dígitos → el gate muestra "9000 €" (no "9.000 €"); comportamiento real documentado en test.
- **Deuda conocida (no tocada):** `deno check` falla en `whatsapp-agent`/`chat-with-hero` por los tipos de supabase-js 2.110 con `ReturnType<typeof createClient>` — **preexistente** (el deploy empaqueta sin tipar; verificado contra HEAD). Los módulos `_shared` nuevos tipan limpio (deno test los chequea).
- **⏳ PENDIENTE al cierre (orden): (1) aplicar `supabase/sql/2026-07-06-app-config.sql` (verificar que `herohome_api_key` sale con 4 caracteres + longitud, no vacía); (2) re-ejecutar `setup-crons.sql` ENTERO (ya es seguro); (3) push a `main`; (4) opcional: dar de alta los checks en Healthchecks.io y pegar las URLs en `app_config`.**

### 5 julio 2026 — Sección "Mi Equipo" (PWA) + robustez del agente WhatsApp (P1–P5) — ✅ DESPLEGADO (SQL aplicado, crons arreglados vía cron.alter_job tras el incidente del placeholder, push + deploy verde)
...
**Monitoring v1 (tarea de B12) — HECHO (alertas por email):** helper `_shared/alert.ts` → `alertTeam({source, subject, detail})` manda email de alerta a `hola@herohome.es` vía Resend (best-effort, nunca lanza). Cableado en los **3 crons que son Edge Functions**: `visit-reminders`, `post-visit-followup`, `generate-slots`. Alerta ante: (a) **excepción no controlada** (los 2 primeros envueltos en `handle()` + try/catch; `generate-slots` ya tenía su try/catch fatal), (b) **fallo de la query** de la BD, (c) **fallos de envío agregados** (WhatsApp/email que fallan → un email con el listado, no uno por fallo). Enfoque elegido por el usuario: solo email (sin Sentry ni dead-man's-switch por ahora). **Limitaciones conocidas:** `complete-visits` y `cleanup-old-slots` son **SQL inline en pg_cron** (no Edge Functions) → no se pueden cablear desde código; y esto NO detecta "el cron **no llegó a correr**" (para eso haría falta un dead-man's-switch externo tipo Healthchecks.io — pendiente si se quiere). Funciones user-facing (notify-visit, create-offer, manage-offer…) no cableadas: sus fallos ya afloran al usuario en la PWA.

**Pen test de endpoints (tarea de B12) — HECHO (sin agujeros):** batería con `curl` contra producción. **HMAC** (`whatsapp-agent`, verify_jwt=false): POST sin firma / firma malformada / firma falsa → 403; GET con verify_token incorrecto/vacío → 403 (no se puede inyectar un mensaje de WhatsApp falso). **x-api-key** (`process-idealista-lead`, `visit-reminders`, `generate-slots`, `post-visit-followup`): 401 sin key o con key incorrecta. **JWT** (`chat-with-hero`, `manage-visit`, `manage-offer`, verify_jwt=true): 401 sin token, y también 401 con **anon key pero sin usuario real** (la función valida el JWT del usuario, no solo el gateway). Verificado que los rechazos no crearon datos. **Pendiente (requiere datos reales):** test de **aislamiento cruzado** (un CV actuando sobre la vivienda de otro CV) — necesita 2 sesiones válidas.
...
- **Resto de campos de propiedad (hecho en esta misma sesión):** `create-user` ahora persiste también `description`, `age`, `communityFee→community_fee`, `condition`, `electronicCertificate→electronic_certificate`, `external→is_exterior`, `garageSpace→garage_space`, `heatingType→heating_type`, `elevator→has_elevator`, `orientation`, `refCatastral→ref_catastral`, `registroPropiedad→registro_propiedad`, `rejectOffersBelow→reject_offers_below` y `floor` (antes el Apex los enviaba pero se ignoraban). `high` (`High__c`) NO tiene columna en `properties` → se omite. **Verificado e2e (30 jun, vivienda SF `001Pw00000weWKvIAM`):** los picklists de SF (condition `Nueva o recién reformada`, orientation `Este`, heating_type `Central`, garage_space `Con 2 plazas de garaje`, electronic_certificate `D`, housing_type `Chalet`) **coinciden** con las etiquetas de los `SelectField` de la PWA; booleanos (`has_elevator`/`is_exterior`) y numéricos OK; fees exactos (1,1%→4.400 €, 1,5%→6.000 €).

**Desajustes SF↔PWA menores conocidos (documentados, no corregidos — decisión del usuario):**
- **`state`:** SF manda el **código** de provincia (`"M"`, `"S"`…), no el nombre. La PWA muestra el código crudo en "Provincia". `notify-visit` ya lo evita componiendo la dirección solo con calle+ciudad. Si molesta, mapear código→nombre en el Apex o en `create-user`.
- **`age`:** SF manda un **texto de rango** (`"70 a 100 años"`); la columna es `text` y guarda bien, pero el tipo `Property` de la PWA declara `age` como número. Alinear (tratar `age` como texto libre en la PWA) si se quiere mostrar/editar.
- **`status`:** SF manda `"On sale"` (minúscula); el tipo de la PWA usa `'On Sale'`. Inconsistencia **preexistente** (los crons filtran por `"On sale"`), ajena a este cambio.
...
- **Plantillas Meta:** `oferta_aceptada`/`oferta_no_aceptada`/`contraoferta` aprobadas; `post_visita` ({{1}} nombre, {{2}} dirección) en revisión. La de rechazo es **`oferta_no_aceptada`** (categoría **Servicio**): sustituye a la antigua `oferta_rechazada`, que Meta recategorizaba a Marketing y no se entregaba fuera de la ventana de 24h. Regla: las plantillas de rechazo van sin CTA de reenganche para no caer en Marketing.
...
- **Endurecido (jul-2026, tras incidencia de rechazo no entregado):** (1) plantilla de rechazo migrada a `oferta_no_aceptada` (Servicio) — ver arriba; (2) `create-offer` cierra la oferta previa del comprador en `Presented` (anti-duplicado); (3) `manage-offer` registra el aviso en `whatsapp_conversations` (visible en el panel) y **resalta en el email al equipo si el WhatsApp al comprador no se entrega** (antes era un fallo silencioso). Ver `docs/B9-OFERTAS.md` §10.
